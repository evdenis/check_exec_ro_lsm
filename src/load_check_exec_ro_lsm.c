...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
	...
	printf("%-8s %-7d %-16s 0%-3o %s\n", ts, e->pid, e->comm, e->mode, e->filename);
...
int main(int argc, char *argv[])
{
	...
	obj = check_exec_ro_lsm__open_and_load();
	...
	err = check_exec_ro_lsm__attach(obj);
	check_fail(err, cleanup, "Failed attach to BPF program");

	printf("%-8s %-7s %-16s %-3s %s\n",
	       "TIME", "PID", "PROCESS", "MODE", "CALLING");
	while (!exiting) {
	...
	check_exec_ro_lsm__detach(obj);

cleanup:
	ring_buffer__free(rb);
	check_exec_ro_lsm__destroy(obj);
